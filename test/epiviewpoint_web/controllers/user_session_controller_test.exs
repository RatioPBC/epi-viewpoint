defmodule EpiViewpointWeb.UserSessionControllerTest do
  ...
  describe "new when there are no existing users" do
    setup do
      ...
      on_exit(fn ->
        Application.put_env(:epiviewpoint, :initial_user_email, nil)
      end)
    end
    ...
    test "when initial_user_email is set, a new initial admin user is created", %{conn: conn} do
      Application.put_env(:epiviewpoint, :initial_user_email, "initial@example.com")
    ...
    test "when initial_user_email is set but user could not be created, an error is shown", %{conn: conn} do
      Application.put_env(:epiviewpoint, :initial_user_email, "invalid email address")
