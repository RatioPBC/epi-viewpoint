defmodule EpicenterWeb.FormHelpersTest do
  ...
  describe "radio_button_list" do
    test "renders radio buttons in reverse order, which get un-reversed via css", %{form: form} do
      ...
      <div class="radio-button-list" id="language-list">
        ...
        <label><input id="movie_language_German" name="movie[language]" type="radio" value="German" checked="checked"/> German</label>
    ...
    test "optionally includes an 'other' option with an associated text input", %{form: form} do
      ...
      |> assert_html_eq("""
      <div class="radio-button-list" id="language-list">
        <label><input id="movie_language_" name="movie[language]" type="radio" value="" checked="checked"/> Other<input data-reveal="when-parent-checked" id="movie_language" name="movie[language]" type="text" value="German"></label>
        <label><input id="movie_language_Italian" name="movie[language]" type="radio" value="Italian"/> Italian</label>
        <label><input id="movie_language_English" name="movie[language]" type="radio" value="English"/> English</label>
      </div>
      """)
  ...
  describe "select_with_wrapper" do
    test "renders a select inside a div with 'select-wrapper' class and an icon", %{form: form} do
      ...
      FormHelpers.select_with_wrapper(form, :language, options)
      |> assert_html_eq("""
      <div class="select-wrapper" data-grid-col="1" data-grid-row="3" data-grid-span="2">
        ...
        <select id="movie_language" name="movie[language]">
