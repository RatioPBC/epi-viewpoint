defmodule EpicenterWeb.ContactsLiveTest do
  ...
  describe "rendering" do
    ...
    test "People with  exposures are listed", %{conn: conn, exposure: exposure} do
      ...
      |> Pages.Contacts.assert_table_contents([
        ...
        ["", "Caroline Testuser", exposure.exposed_person.id, "10/31/2020", "", "assignee"]
      ])
    end
  end
  ...
  defp create_contacts(%{user: user} = _context) do
    ...
    Cases.assign_user_to_people(user_id: assignee.id, people_ids: [exposure.exposed_person.id], audit_meta: Test.Fixtures.admin_audit_meta())
