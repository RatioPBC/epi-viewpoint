defmodule EpiViewpoint.AuditingRepoTest do
  ...
  describe "inserting" do
    ...
    test "omits passwords from the revision" do
      email = EpiViewpoint.AccountsFixtures.unique_user_email()
      password = EpiViewpoint.AccountsFixtures.valid_user_password()
      {user_attrs, _} = Test.Fixtures.user_attrs(%{id: ""}, "user", email: email, password: password)
      password_changeset = %EpiViewpoint.Accounts.User{} |> EpiViewpoint.Accounts.User.registration_changeset(user_attrs)
    ...
    test "omits mfa_secret from the revision" do
      mfa_secret = "123456"
      user = Test.Fixtures.user_attrs(@admin, "user") |> Accounts.register_user!()
      mfa_changeset = user |> EpiViewpoint.Accounts.User.mfa_changeset(%{"mfa_secret" => mfa_secret})
