defmodule EpiViewpoint.AccountsTest do
  ...
  describe "user creation" do
    ...
    @ops_admin %EpiViewpoint.Accounts.User{id: Application.compile_env(:epiviewpoint, :unpersisted_admin_id)}
    ...
    test "register_user! fails when originator is not admin" do
      ...
      assert_raise EpiViewpoint.AdminRequiredError, fn ->
  ...
  describe "update_user/2" do
    ...
    test "fails when originator is the unpersisted admin (that's only supposed to be used for creating users)", %{user: user} do
      audit_meta = Test.Fixtures.audit_meta(%{id: Application.get_env(:epiviewpoint, :unpersisted_admin_id)})
  ...
  describe "create_login" do
    test "adding a single row for the audit trail when a user logs in" do
      ...
      assert Euclid.Exists.present?(login.inserted_at)
    end
  end
  ...
  describe "list_recent_logins" do
    test "only fetches the logins for the requested user_id" do
      ...
      Accounts.create_login(%{
        ...
        tid: "login2",
        user_agent: "something complicated2",
        user_id: user2.id
      })
