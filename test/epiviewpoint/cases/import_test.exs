defmodule EpiViewpoint.Cases.ImportTest do
  use EpiViewpoint.DataCase, async: true

  import Euclid.Extra.Enum, only: [pluck: 2, tids: 1]
  import EpiViewpoint.Extra.String, only: [add_numeric_suffix: 1]
  ...
  describe "happy path" do
    test "creates LabResult records and Person records from csv data", %{originator: originator} do
      assert {:ok,
              %EpiViewpoint.Cases.Import.ImportInfo{
               ...
               %{
                 file_name: "test.csv",
                 contents: """
                 search_firstname_2 , search_lastname_1 , dateofbirth_8 , phonenumber_7 , caseid_0 , datecollected_36 , resultdate_42 , result_39 , orderingfacilityname_37 , lab_result_tid , diagaddress_street1_3 , diagaddress_city_4 , diagaddress_state_5 , diagaddress_zip_6 , datereportedtolhd_44 , testname_38 , person_tid, sex_11, ethnicity_13, occupation_18   , race_12
                 Alice              , Testuser          , 01/01/1970    , 1111111000    , 10000    , 06/01/2020       , 06/03/2020    , positive  , Lab Co South            , alice-result-1 ,                       ,                    ,                     ,                   , 06/05/2020           , TestTest    , alice     , female, HispanicOrLatino       , Rocket Scientist, Asian Indian
      ...
      assert {:ok,
              %EpiViewpoint.Cases.Import.ImportInfo{
               ...
               %{
                 file_name: "test.csv",
                 contents: """
                 search_firstname_2 , search_lastname_1 , dateofbirth_8 , phonenumber_7 , caseid_0 , datecollected_36 , resultdate_42 , result_39 , orderingfacilityname_37 , person_tid , lab_result_tid , diagaddress_street1_3 , diagaddress_city_4 , diagaddress_state_5 , diagaddress_zip_6 , datereportedtolhd_44 , testname_38 , sex_11, ethnicity_13, occupation_18   , race_12
  ...
  describe "de-duplication" do
    test "if two lab results have the same first_name, last_name, and dob, they are considered the same person",
      ...
      [alice, billy_1, billy_2] = Cases.list_people(:all, user: @admin, reject_archived_people: true) |> Enum.map(&Cases.preload_lab_results/1)
