defmodule EpicenterWeb.UsersLive do
  ...
  def handle_event("reset-password", %{"user-id" => user_id}, socket),
    do:
      socket
      |> assign(users: set_password_reset_url(socket.assigns.users, user_id, &password_reset_url/1))
      |> noreply()

  def handle_event("close-reset-password", %{"user-id" => user_id}, socket),
    do:
      socket
      |> assign(users: set_password_reset_url(socket.assigns.users, user_id, fn _ -> nil end))
      |> noreply()
