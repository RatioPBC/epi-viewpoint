defmodule EpicenterWeb.ContactsFilter do
  ...
  def render(assigns) do
    ...
    <div id="status-filter">
      <.link patch={~p"/contacts?filter=with_contact_investigation"} class="button" data-active={to_string(assigns.filter in [:with_contact_investigation, nil])} data-role="contacts-filter" data-tid="all">All</.link>
      <.link patch={~p"/contacts?filter=with_pending_interview"} class="button" data-active={to_string(assigns.filter == :with_pending_interview)} data-role="contacts-filter" data-tid="with_pending_interview">Pending interview</.link>
      <.link patch={~p"/contacts?filter=with_ongoing_interview"} class="button" data-active={to_string(assigns.filter == :with_ongoing_interview)} data-role="contacts-filter" data-tid="with_ongoing_interview">Ongoing interview</.link>
      <.link patch={~p"/contacts?filter=with_quarantine_monitoring"} class="button" data-active={to_string(assigns.filter == :with_quarantine_monitoring)} data-role="contacts-filter" data-tid="with_quarantine_monitoring">Quarantine monitoring</.link>
    </div>
    """
    |> Map.put(:root, true)
  end
