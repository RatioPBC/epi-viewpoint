defmodule EpicenterWeb.InvestigationCompleteInterviewLive do
  ...
  def mount(
        ...
        session,
        %{assigns: %{live_action: :complete_contact_investigation}} = socket
      ) do
  ...
  def mount(
        ...
        session,
        %{assigns: %{live_action: :complete_case_investigation}} = socket
      ) do
    socket = socket |> authenticate_user(session)
  ...
  def handle_event("save", %{"complete_interview_form" => params}, socket) do
    with %Ecto.Changeset{} = form_changeset <-
           CompleteInterviewForm.changeset(socket.assigns.investigation, params),
         {:form, {:ok, case_investigation_attrs}} <-
           {:form, CompleteInterviewForm.investigation_attrs(form_changeset)},
         {:investigation, {:ok, _investigation}} <-
           {:investigation, update_case_investigation(socket, case_investigation_attrs)} do
      socket
  ...
  defp get_person(%ContactInvestigations.ContactInvestigation{} = investigation),
    do: investigation.exposed_person
  ...
  defp update_case_investigation(
         %{assigns: %{investigation: %Cases.CaseInvestigation{} = investigation}} = socket,
         params
       ) do
  ...
  defp update_case_investigation(
         %{
           assigns: %{
             investigation: %ContactInvestigations.ContactInvestigation{} = contact_investigation
           }
         } = socket,
         params
       ) do
    ContactInvestigations.complete_interview(
