defmodule EpiViewpointWeb.Presenters.ContactInvestigationPresenter do
  import EpiViewpointWeb.PersonHelpers, only: [demographic_field: 2]

  use EpiViewpointWeb, :presenter
  ...
  defp quarantine_conclusion(contact_investigation) do
    %{
      text:
        "Concluded quarantine monitoring on #{Format.date(contact_investigation.quarantine_concluded_at)}: #{Gettext.gettext(EpiViewpoint.Gettext, contact_investigation.quarantine_conclusion_reason)}",
      link:
        link_if_editable(
          contact_investigation.exposed_person,
