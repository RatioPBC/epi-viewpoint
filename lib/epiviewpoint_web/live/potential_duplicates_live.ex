defmodule EpiViewpointWeb.PotentialDuplicatesLive do
  use EpiViewpointWeb, :live_view

  import EpiViewpointWeb.IconView, only: [arrow_right_icon: 0, back_icon: 0]
  import EpiViewpointWeb.LiveHelpers, only: [assign_defaults: 2, assign_page_title: 2, authenticate_user: 2, noreply: 1, ok: 1]
  import EpiViewpointWeb.Unknown, only: [list_or_unknown: 2]
  ...
  def handle_event("merge-selected-people", _params, socket) do
    ...
    socket
    |> push_navigate(to: ~p"/people/#{socket.assigns[:person]}/resolve-conflicts?duplicate_person_ids=#{duplicate_person_ids}")
    |> noreply
  end
