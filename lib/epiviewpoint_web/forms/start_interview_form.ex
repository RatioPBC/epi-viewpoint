defmodule EpiViewpointWeb.Forms.StartInterviewForm do
  use Ecto.Schema

  import Ecto.Changeset
  import EpiViewpointWeb.Views.DateExtraction, only: [convert_time: 3, extract_and_validate_date: 4]
  ...
  defp investigation_start_interview_form_attrs(%{interview_started_at: interview_started_at} = investigation) do
    time =
      if interview_started_at do
        Timex.Timezone.convert(interview_started_at, EpiViewpointWeb.PresentationConstants.presented_time_zone())
      else
        Timex.now(EpiViewpointWeb.PresentationConstants.presented_time_zone())
      end
